...
dominio <- seq(-10,10,0.5)

df <- data.frame(
  x = dominio, 
  y = transformacion(dominio), 
  frame = 1:(length(dominio))
)
...
plot_ly(df,
    ...
    frame = ~frame,
    type = 'scatter',
    mode = 'markers', # graficar cada coordenada por separado
    showlegend = FALSE
) %>%
  add_trace(
    marker=list(size=12,color='skyblue'),
    text = ~paste("x: ",round(x,2),"\nf(x): ",round(y,2),sep=''),
    hoverinfo = 'text' # mostrar texto cuando mouse este encima del punto
) %>%
  animation_opts(
    frame = 100,      # tiempo (en ms) entre frames
    transition = 0,   # Duración (en ms) de la transición entre frames
    easing = "linear" # tipo de transición entre frames
)
```
]
...
txhousing %>%
  filter(city %in% c("Abilene", "Bay Area")) %>%
  accumulate_by(~date) %>%
  plot_ly(
    x = ~date, 
    y = ~median,
...
```{r, plotly-3d, include = FALSE, eval = TRUE}
plot_ly(z = ~volcano) %>%
  add_surface(
    contours = list(
                    z = list(
...
```{r custom-graph-1, include = FALSE, eval = TRUE}
